...
//		//算法三
	...
	{
		var E = this;
		var K = this.$k, L = E.$lang, Lg = E.$c.lang, a = [];
		...
		for( var i = 0;i < 7;i++)
		{
			a.push('<th class="title">' + L.w[Lg][i] + '</th>');
		}
		a.push('</tr>');
		for( var i = 0;i < 6;i++)
		{
		...
		a.push('<label' + (E.$c.level > 3 ? ' style="display:none;"' : '') + '><div id="' + K.H + '" class="btn"></div>' + L.t[Lg][3] + '</label>');
		a.push('<label' + (E.$c.level > 2 ? ' style="display:none;"' : '') + '><div id="' + K.m + '" class="btn"></div>' + L.t[Lg][2] + '</label>');
		a.push('<label' + (E.$c.level > 1 ? ' style="display:none;"' : '') + '><div id="' + K.s + '" class="btn"></div>' + L.t[Lg][1] + '</label>');
		a.push('</div>');

		a.push('<div class="dbtn">');
		a.push('<div id="' + K.clear + '" class="btn">' + L.b[Lg][1] + '</div>');
		a.push('<div id="' + K.ok    + '" class="btn">' + L.b[Lg][0] + '</div>');
		a.push('<div id="' + K.close + '" class="btn">' + L.b[Lg][2] + '</div>');
		a.push('</div>');
	...
	{
		var E = this;
		var o = E.$(E.$k.data), L = E.$lang, Lg = E.$c.lang, a = [];
		a.push('<table>');
		if(t < 4 && t > 0)
		{
			var m = 10;//分钟2和秒钟1
		...
		else if(t == 5)// 月5
		{
			...
			for( var i = 0;i < 6;i++)
			{
				...
				for( var j = 0;j < m;j++)
				{
					a.push('<td cv="' + (i*m+j) + '">' + L.M[Lg][(i*m+j)] + '</td>');//cv从0开始
				}
				a.push('</tr>');
			}
		}
		else// 年6
		{
			...
			for( var i = 0;i < 5;i++)
			{
				...
				for( var j = 0;j < m;j++)
				{
					a.push('<td cv="' + (y+i*m+j) + '">' + E.$fnYear(y+i*m+j) + L.y[Lg] + '</td>');
				}
				a.push('</tr>');
			}
			a.push('<tr>');
		...
		a.push('</table>');
		E.$(E.$k.title).innerHTML = L.f[Lg][t];
		o.innerHTML = a.join("");
	...
	$recovery:function()
	{
		...
		E.$(K.y).innerHTML = E.$fnYear(S.y) + E.$lang.y[E.$c.lang];
		if(E.$c.level >= 6)
		{
			return;
		}
		E.$(K.M).innerHTML = E.$lang.M[E.$c.lang][S.M];
